...
namespace BLTAdoptAHero
{
    ...
    public class BLTTournamentMissionBehavior : AutoMissionBehavior<BLTTournamentMissionBehavior>
    {
        ...
        {
            // Class Loadout / Cultural Unified set weapons per-participant in AddRandomClothes,
            // so skip the shared weapon-list manipulation entirely for those modes.
            if (BLTAdoptAHeroModule.TournamentConfig.LoadoutMode != TournamentLoadoutMode.Default)
                return;
        ...
        // Cultural Unified mode: weapon type is chosen once per round and shared by all fighters
        private int _unifiedRoundIndex = -1;
        private EquipmentType _unifiedWeapon = EquipmentType.OneHandedSword;

        private EquipmentType GetUnifiedWeaponForCurrentRound()
        {
            var pool = BLTAdoptAHeroModule.TournamentConfig.UnifiedWeaponPool;
            if (pool == null || pool.Count == 0)
                return EquipmentType.OneHandedSword;

            int round = 0;
            try
            {
                round = MissionState.Current?.CurrentMission?
                    .GetMissionBehavior<TournamentBehavior>()?.CurrentRoundIndex ?? 0;
            }
            catch { /* fall back to round 0 */ }

            if (round != _unifiedRoundIndex)
            {
                _unifiedRoundIndex = round;
                _unifiedWeapon = pool[MBRandom.RandomInt(pool.Count)];
            }
            return _unifiedWeapon;
        }

        // Set all armour slots to a fixed tier, preferring the participant's OWN culture (e.g. an orc
        // hero gets orc armor even when fighting in a human tournament host settlement); falls back to
        // the host settlement's culture for regular troops with no Hero, then to any culture at all.
        private void ApplyNormalizedArmor(CultureObject culture, TournamentParticipant participant, int tierOneBased)
        {
            int tier = Math.Max(0, Math.Min(7, tierOneBased - 1));
            var heroCulture = participant.Character?.HeroObject?.Culture;
            foreach (var (slot, itemType) in SkillGroup.ArmorIndexType)
            {
                var item = (heroCulture != null
                               ? EquipHero.SelectRandomItemNearestTier(
                                   CampaignHelpers.AllItems.Where(i => i.Culture == heroCulture && i.ItemType == itemType), tier)
                               : null)
                           ?? EquipHero.SelectRandomItemNearestTier(
                               CampaignHelpers.AllItems.Where(i => i.Culture == culture && i.ItemType == itemType), tier)
                           ?? EquipHero.SelectRandomItemNearestTier(
        ...
        // Pick a single item matching an equipment type at the nearest tier, preferring a culture if given
        private static ItemObject PickItemOfType(EquipmentType type, int tier, CultureObject culture)
        {
            if (type == EquipmentType.None) return null;
            IEnumerable<ItemObject> items = CampaignHelpers.AllItems.Where(i => i.IsEquipmentType(type));
            if (culture != null)
            {
                var cultItems = items.Where(i => i.Culture == culture).ToList();
                if (cultItems.Count > 0) items = cultItems;
            }
            return EquipHero.SelectRandomItemNearestTier(items, tier);
        }

        private static ItemObject PickItemByItemType(ItemObject.ItemTypeEnum itemType, int tier, CultureObject culture)
        {
            IEnumerable<ItemObject> items = CampaignHelpers.AllItems.Where(i => i.ItemType == itemType);
            if (culture != null)
            {
                var cultItems = items.Where(i => i.Culture == culture).ToList();
                if (cultItems.Count > 0) items = cultItems;
        ...
        {
            var cfg = BLTAdoptAHeroModule.TournamentConfig;

            // Default mode: keep the original vanilla / Normalize Armor behaviour
            if (cfg.LoadoutMode == TournamentLoadoutMode.Default)
            {
                if (cfg.NormalizeArmor)
                {
                    ApplyNormalizedArmor(culture, participant, cfg.NormalizeArmorTier);
                    return true;
                }
                return false;
            }

            int tier = Math.Max(0, Math.Min(7, cfg.LoadoutTier - 1));
            var hero = participant.Character?.HeroObject;

            // Armour: fixed Loadout Tier, cultural where possible (both custom modes)
            ApplyNormalizedArmor(culture, participant, cfg.LoadoutTier);

            // Clear weapon + mount slots so we control them fully
            foreach (var (_, index) in participant.MatchEquipment.YieldWeaponSlots())
                participant.MatchEquipment[index] = EquipmentElement.Invalid;
            participant.MatchEquipment[EquipmentIndex.Horse] = EquipmentElement.Invalid;
            participant.MatchEquipment[EquipmentIndex.HorseHarness] = EquipmentElement.Invalid;

            if (cfg.LoadoutMode == TournamentLoadoutMode.ClassLoadout)
                BuildClassLoadout(participant, hero, tier);
            else
                BuildUnifiedLoadout(participant, culture, tier);

            return true;
        }

        // Mode 2: each hero uses their class weapon slot types at the fixed tier (mounted classes get a horse)
        private void BuildClassLoadout(TournamentParticipant participant, Hero hero, int tier)
        {
            var weaponSlots = participant.MatchEquipment.YieldWeaponSlots().Select(s => s.index).ToList();
            int slotPtr = 0;
            var classDef = hero?.GetClass();

            // Match the hero's OWN culture where possible (e.g. an orc hero gets orc weapons, not
            // whatever the tournament host settlement happens to be) - previously this was hardcoded
            // to null, so class-loadout tournament rewards ignored culture entirely.
            var heroCulture = hero?.Culture;

            if (classDef?.SlotItems != null)
            {
                foreach (var equipmentType in classDef.SlotItems)
                {
                    if (equipmentType == EquipmentType.None) continue;
                    if (slotPtr >= weaponSlots.Count) break;
                    var item = PickItemOfType(equipmentType, tier, heroCulture);
                    if (item != null)
                        participant.MatchEquipment[weaponSlots[slotPtr++]] = new EquipmentElement(item);
                }

                if (classDef.Mounted)
                {
                    var horse = PickItemByItemType(ItemObject.ItemTypeEnum.Horse, tier, heroCulture);
                    if (horse != null)
                    {
                        participant.MatchEquipment[EquipmentIndex.Horse] = new EquipmentElement(horse);
                        var harness = PickItemByItemType(ItemObject.ItemTypeEnum.HorseHarness, tier, heroCulture);
                        if (harness != null)
                            participant.MatchEquipment[EquipmentIndex.HorseHarness] = new EquipmentElement(harness);
                    }
                }
            }
            else if (weaponSlots.Count > 0)
            {
                // No class assigned: fall back to a simple sword
                var item = PickItemOfType(EquipmentType.OneHandedSword, tier, heroCulture);
                if (item != null)
                    participant.MatchEquipment[weaponSlots[0]] = new EquipmentElement(item);
            }
        }

        // Mode 3: everyone uses the round's weapon type, but from their own culture; on foot
        private void BuildUnifiedLoadout(TournamentParticipant participant, CultureObject culture, int tier)
        {
            var weaponSlots = participant.MatchEquipment.YieldWeaponSlots().Select(s => s.index).ToList();
            if (weaponSlots.Count == 0) return;

            var weaponType = GetUnifiedWeaponForCurrentRound();
            var weapon = PickItemOfType(weaponType, tier, culture)
                         ?? PickItemOfType(EquipmentType.OneHandedSword, tier, culture);
            if (weapon == null) return;

            int slotPtr = 0;
            participant.MatchEquipment[weaponSlots[slotPtr++]] = new EquipmentElement(weapon);
            ...
            {
                if (slotPtr < weaponSlots.Count)
                {
                    var ammo = PickItemByItemType(ammoType, tier, culture);
                    if (ammo != null)
                        participant.MatchEquipment[weaponSlots[slotPtr++]] = new EquipmentElement(ammo);
                }
            }
            else if (EquipHero.WeaponIsSwingable(weapon)
                     && !EquipHero.WeaponRequires(weapon, ItemObject.ItemUsageSetFlags.RequiresNoShield)
                     && slotPtr < weaponSlots.Count)
            {
                // One-handed swingable weapon: give a matching-culture shield
                var shield = PickItemOfType(EquipmentType.Shield, tier, culture);
                if (shield != null)
                    participant.MatchEquipment[weaponSlots[slotPtr++]] = new EquipmentElement(shield);
            }
        }
