...
namespace BLTAdoptAHero
{
    public class BLTCustomItemsCampaignBehavior : CampaignBehaviorBase
    {
        ...
        private class ItemModifierData
        {
            ...
            // Gold actually spent creating this item (Smith/Enchant); 0 for free rewards (tournament
            // prizes etc). Used to work out a partial refund when the item is later discarded.
            [UsedImplicitly]
        ...
        public void SetPurchaseCost(ItemModifier modifier, int cost)
        {
            if (modifier != null && customItemModifiers.TryGetValue(modifier, out var data))
                data.PurchaseCost = cost;
        }

        public int GetPurchaseCost(ItemModifier modifier)
            => modifier != null && customItemModifiers.TryGetValue(modifier, out var data) ? data.PurchaseCost : 0;
