...
namespace BLTAdoptAHero.Models
{
    ...
    public class BLTPartySizeLimitModel : PartySizeLimitModel
    {
        ...
        // Gdy Warsails/NavalDLC jest WŁĄCZONY — oddaj sterowanie oryginalnemu modelowi
        // (partie dostają normalnie statki). Pustą listę zwracamy TYLKO bez Warsails
        // (wariant NonWarsails). Bez tego korsarze/floty spawnują się bez statku.
        public override List<TaleWorlds.CampaignSystem.Naval.Ship> FindAppropriateInitialShipsForMobileParty(MobileParty party, PartyTemplateObject partyTemplate)
            => BannerlordTwitch.Helpers.CampaignHelpers.NavalDLC()
                ? _previous.FindAppropriateInitialShipsForMobileParty(party, partyTemplate)
                : new List<TaleWorlds.CampaignSystem.Naval.Ship>();
