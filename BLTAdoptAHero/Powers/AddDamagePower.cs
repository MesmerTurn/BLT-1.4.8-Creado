...
namespace BLTAdoptAHero.Powers
{
    ...
    public class AddDamagePower : DurationMissionHeroPowerDefBase, IHeroPowerPassive, IDocumentable
    {
        ...
        private bool IgnoreDamageType(Agent attackerAgent, Agent victimAgent, AttackCollisionData attackCollisionData)
        {
            ...
            // Safely determine if the attacker is wielding a consumable (throwing/ammo) weapon.
            // When the attacker has nothing wielded (kick, unarmed, power-based hit) the
            // WieldedWeapon getter can throw a NullReferenceException, so guard it.
            bool wieldedConsumable = false;
            try
            {
                if (attackerAgent != null)
                {
                    var wielded = attackerAgent.WieldedWeapon;
                    wieldedConsumable = !wielded.IsEmpty && wielded.IsAnyConsumable();
                }
            }
            catch
            {
                wieldedConsumable = false;
            }
            ...
            return attackCollisionData.IsFallDamage
                   ...
                   || !Melee && !((attackCollisionData.IsMissile || wieldedConsumable) || attackCollisionData.IsHorseCharge)
                   || !Ranged && (attackCollisionData.IsMissile || wieldedConsumable)
